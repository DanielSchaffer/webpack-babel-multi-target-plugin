...
export function HostListenerNoNgZone(eventName: string, args?: string[]) {
    let parts = eventName.split(':');
    let target = parts.length === 2 ? parts[0] : 'window';
    let actualEventName = parts.length === 2 ? parts[1]: parts[0];
